...
public class PullToRefreshView extends LinearLayout {
    ...
    private OnHeaderRefreshStateChangeListener mOnHeaderRefreshStateChangeListener;
    /**
     * last y
     */
            ...
            if (view instanceof RecyclerView) {
                ...
                mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
                    ...
                    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                        ...
                        if (mLoadMoreEnable && mPullState == PULL_UP_STATE && newState == RecyclerView.SCROLL_STATE_IDLE && dy > delata) {
                            if (isRefreshViewScroll(-delata - 1) && mFooterState != REFRESHING) {
...
//                    Log.i(TAG, "pull up!parent view move!" + mFooterState);
                    if (getScrollY() > 0 || deltaY < 0) {
...
//                    LogUtils.d("health_cloud" + mFooterState);
    ...
    private boolean isRefreshViewScroll(int deltaY) {
        ...
        if (mRecyclerView != null) {
            if (deltaY > delata) {
                ...
                int top = child.getTop() - ((RecyclerView.LayoutParams)child.getLayoutParams()).topMargin;
...
//                ((AnimationDrawable) mHeaderImageView.getDrawable()).start();
            ...
            mHeaderDrawable.selectDrawable(mHeaderDrawable.getNumberOfFrames() - 1 -
                    mHeaderDrawable.getNumberOfFrames() * (mHeaderViewHeight - Math.abs(newTopMargin)) / mHeaderViewHeight);
            mHeaderState = PULL_TO_REFRESH;
        }
        if (mOnHeaderRefreshStateChangeListener != null) {
            mOnHeaderRefreshStateChangeListener.onPrepareToRefresh();
        }
    }
...
//        mHeaderProgressBar.setVisibility(View.VISIBLE);
        ...
        if (mOnHeaderRefreshStateChangeListener != null) {
            mOnHeaderRefreshStateChangeListener.onRefreshing();
        }
    }

    private void footerRefreshing() {
        boolean isRefreshing = (mFooterState == REFRESHING);
        ...
        if (mOnFooterRefreshListener != null && !isRefreshing) {
...
//        ((AnimationDrawable)mHeaderImageView.getDrawable()).stop();
        ...
        mHeaderState = PULL_TO_REFRESH;
        if (mOnHeaderRefreshStateChangeListener != null) {
            mOnHeaderRefreshStateChangeListener.onRefreshFinish();
        }
    }
    ...
    private SimpleDateFormat mSimpleFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.getDefault());
    ...
    private String getTransforDate(long time) {
        return "今天 " + mTimeFormat.format(new Date(time));
//        String nowStr = mSimpleFormat.format(new Date(time));
//        try {
//            Date now = new Date(time);
//            Date today = mSimpleFormat.parse(nowStr);
//            Date yesterday = new Date(today.getTime() - 24 * 3600);
//            Date qianday = new Date(yesterday.getTime() - 24 * 3600);
//            if (now.after(today)) {
//                return mTimeFormat.format(new Date(time));
//            } else if (now.after(yesterday)) {
//                return "昨天" + mTimeFormat.format(new Date(time));
//            } else if (now.after(qianday)) {
//                return "前天" + mTimeFormat.format(new Date(time));
//            } else {
...
//        return mSimpleFormat.format(new Date(time));
    ...
    public interface OnPullBaseScrollChanged {
        void onPullBaseScrollChanged(int l, int t, int oldl, int oldt);
    }
    ...
    public interface OnHeaderRefreshStateChangeListener{
        void onPrepareToRefresh();
        void onRefreshing();
        void onRefreshFinish();
    }

    public void setOnHeaderRefreshStateChangeListener(OnHeaderRefreshStateChangeListener onHeaderRefreshStateChangeListener) {
        mOnHeaderRefreshStateChangeListener = onHeaderRefreshStateChangeListener;
    }
}
