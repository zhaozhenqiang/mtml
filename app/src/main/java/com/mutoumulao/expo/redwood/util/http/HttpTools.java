...
public class HttpTools {
    ...
    private OkHttpClient initOkHttpClient() {
        ...
        httpLoggingInterceptor.setLevel(Allow_Log ? HttpLoggingInterceptor.Level.BODY : HttpLoggingInterceptor.Level.NONE);
    ...
    public <T> CallProxy put(RequestParams params, BaseCallback<T> callBack) {
        params.setMethod(HttpMethod.PUT);
        return send(params, callBack);
    }
        ...
        try {
            if (stream != null && stream.isSuccessful()) {
                ...
                if (response.isSuccess()) {
                    T t = JSON.parseObject(response.data, clazz);
                    return t;
                }
            }
        } catch (Exception e) {
    ...
    public <T> List<T> sendArraySync(RequestParams params, Class<T> clazz) throws IOException {
        ...
        try {
            if (stream != null && stream.isSuccessful()) {
                ...
                BaseResponse response = JSON.parseObject(result, BaseResponse.class);
                if (response.isSuccess()) {
                    List<T> t = JSON.parseArray(response.data, clazz);
                    return t;
                }
            }
        } catch (Exception e) {
